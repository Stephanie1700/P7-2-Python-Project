...
# ----------------------------------------------------
def load_english_words(filename="englishWords.txt"): #Ensure the file path is correct
...
# -----------------------------
# This function will segment a sentence without spaces into valid english words based on the dictionary of words.
def segment(sentence, segmentation_words):
    results = []

    # Function to try to segment the sentence  (Recursive function) 
    def find_segments(sub_sentence, current_segment):
        # base case: if the sub_sentence is empty, add the possible segmentation to results
        if not sub_sentence:
